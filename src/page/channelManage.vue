  ...
  <div>
    ...
    <div class="content">
      <div class="deleteChannel">
        ...
        <ul>
          <li v-for="(item, index) in channels" class="channelCss" :key="item" @click="deleteChannel(item, index)">{{item}}</li>
        </ul>
      </div>
      <div class="addChannel">
        <div class="title addTitle">点击添加以下频道</div>
        <ul>
          <li v-for="(item, index) in addChannel" class="channelCss" :key="item" @click="appendChannel(item, index)">{{item}}</li>
        </ul>
      </div>
    </div>
  </div>
