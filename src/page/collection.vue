<template lang="html">
  <div>
    ...
    <div class="collection-content">
      <div class="new-container" v-for="(item, index) in collectionData" :key="item.title">
        <div class="cancel">
        ...
        <div class="section" @click="toCollectDetail(item, index)">
          ...
          <div class="new-right">
            ...
            <div class="new-message">
              <span>{{item.time || '2019-12-17 12:00'}}</span>
              <span>{{item.src || 'lastest 40'}}</span>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    deleteCollect(item, index) {
      ...
      window.localStorage.setItem('likelist',JSON.stringify(this.$store.state.collection))
    }
  }
}
</script>
